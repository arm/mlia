...
Provide a shared pytest-native end-to-end testing helper that lets MLIA
repositories define reusable CLI execution suites without duplicating case
expansion, artifact staging, backend setup, and command execution mechanics.
...
variables instead of repo-specific code. It SHALL load execution definitions from
`MLIA_E2E_EXECUTIONS`, prepared artifact roots from `MLIA_E2E_ARTIFACTS`,
...
- **THEN** the helper SHALL fail with an invalid-command runtime error instead of
...
output assertions that are not reusable across MLIA repositories SHALL remain in
the consuming test modules.
